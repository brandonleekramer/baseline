...
colorblock_byppt <- combined_data %>% 
  ...
  filter(segment == "A1") %>% 
  ggplot(aes(x=segment_number, y=rsa, group=subject_id)) + geom_line() +
...
byseg_graph <- ggplot(byseg, aes(x=segment, y=rsa, group=condition, color=condition)) + geom_line(size = 1.3) +
  ggtitle("Mean RSA Variation Over Experiment") + ylab("RSA") + 
